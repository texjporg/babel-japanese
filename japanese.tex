...
required in the input.\footnote{For example, \pdfTeX\ cannot
insert a quarter em space (\Foreign{shibu aki}) automatically, so
the active character \code{\~} is redefined to a macro to
insert it manually. \XeTeX\ has more advanced capability than
\pdfTeX, but interfaces for Japanese \LaTeX\ users (packages, etc.)
are limited.}
\begin{lstlisting}
