...
%% ちゃんと日本語を組むには？ -- 禁則文字を除く任意箇所での
\section{What is the minimum requirement for Japanese typesetting?}

The sections above described practical ways to achieve ``acceptable''
Japanese typesetting results.
...
simple and short horizontal writing.
...
\begin{itemize}
  \item Japanese text has no interword spaces, but
    ...
    (except before some punctuation marks).
    The overfull rules that appeared in Figure \ref{fig:src1} comes
  ...
  \item It is preferred to insert a quarter space (\emph{shibuaki})
...
\begin{figure}[tbp]
\centering\leavevmode\box\JEXBOX
