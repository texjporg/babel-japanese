...
Japanese texts can be aligned vertically, called \emph{tate-gumi} (縦組).
The horizontal alignment of texts is called \emph{yoko-gumi} (横組).
The list below shows \ClsT{\emph{tate} classes in green}
and \ClsYT{\emph{yoko}/\emph{tate} classes in blue} (default is \emph{yoko}
and class option \Opt{[tate]} enables \emph{tate} mode).
